...
- **`tests/` was never type-checked** (#286): `tsconfig.json` sets
  `"rootDir": "src"`, Playwright and vitest transpile without checking, and
  eslint is not type-aware — so a type-broken spec could not fail any gate.
  #284 had introduced three such errors, caught only by an out-of-band `tsc`
  run. CI also inlined its own `tsc` call rather than invoking the npm script,
  so widening the script alone would have left CI checking `src` only. The one
  error this exposed was real: a spec compared a revision against a possibly
  `undefined` value, so the assertion would have passed vacuously.

- **The perf gate could no longer fail** (#278): the committed baseline was
  captured on a workstation in June and never re-measured on a runner, while
  the gate it drives compares CI runs. Filed as flakiness, and that was
  accurate in July — but the Monaco local-AMD refactor (#272) removed a runtime
  jsDelivr fetch on 2026-08-08 and cut `appBootstrap` ~47%, after which the
  624ms budget became unreachable. Re-baselined from 15 CI runs; `appBootstrap`
  tightens to 367ms. Five diagnostic metrics regressed between July and August
  and are baselined in deliberately (#293).

- **Float `0..1` OFF face colors read as near-black** (#287): every channel was
  divided by 255, so `0.5` became `0.00196`. The OFF spec distinguishes the two
  encodings by token type rather than value, and `parts.map(Number)` discarded
  that before the decision was made. Third-party OFF only; OpenSCAD writes
  integers.

- **CGAL silently dropped a model's colors** (#283): only the Manifold backend
  writes per-face colors, so switching backends reverted a `color()` model to
  the default cameo yellow with nothing explaining why. The limitation is
  deliberate; the silence was not.

- **Build and runner configs were unchecked** (#295, #297): five of the eight
  root-level `.ts` files were in no type-check program, and the three that were
  covered were named individually — so a new config stayed unchecked until
  someone remembered to add it. Two real errors in `vite.shared.ts` blocked the
  widening and are fixed at the source.

### Changed

- `perf-baseline.json` is now generated by `scripts/perf/refresh-baseline.mjs`
  (`npm run perf:refresh`) from many harvested CI runs rather than by hand, and
  captures stamp `environment.runId` so a baseline's provenance can be checked
  against the Actions API instead of trusted (#296). The script refuses inputs
  that were not captured on CI, and refuses to write a baseline missing any
  gated metric — both routes by which the gate could silently stop gating.

- Gates are now guarded against silently ceasing to check: the type-check
  program is asserted by asking `tsc --listFiles` what it actually covers
  rather than by reading config text, and every CI job is asserted to be gated
  by the `ok` aggregator, whose `needs:` list was previously kept correct by a
  comment (#298).
