...
describe('e2e', () => {
  ...
  test(
    'customizer & windows line endings work',
    async () => {
      await loadSrc(['myVar = 10;', 'cube(myVar);'].join('\r\n'));
      await waitForViewer();
      await expect3DPolySet();
...
describe('worker integration', () => {
  test(
    'compiles a trivial model successfully (exit code 0)',
    async () => {
  ...
  test(
    'single render produces one stable output artifact',
    async () => {
...
describe('conformance — geometry primitives', () => {
  ...
  test(
    'sphere(5, $fn=20) produces a PolySet',
    async () => {
  ...
  test(
    'cylinder(h=10, r=5) produces a PolySet',
    async () => {
  ...
  test(
    'difference of cube and sphere produces a manifold',
    async () => {
  ...
  test(
    'viewer canvas is populated after compile',
    async () => {
      await loadSrc('cube(10);');
      await waitForViewer();
...
describe('e2e — keyboard shortcuts', () => {
  test(
    'pressing F5 after a render triggers a new render',
    async () => {
      await loadSrc('cube(5);');
      await waitForViewer();
  ...
  test(
    'pressing F6 after a render triggers a full render',
    async () => {
      await loadSrc('cube(5);');
      await waitForViewer();
      const before = await getAppShellState();
