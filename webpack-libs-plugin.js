...
class OpenSCADLibrariesPlugin {
    ...
    async createZip(sourceDir, outputPath, includes = [], excludes = [], workingDir = '.', includeAssets = []) {
        ...
        for (const absPath of allFiles) {
            ...
            if (isAsset || (this.matchesAnyInclude(relPath, effectiveIncludes) && !this.matchesAnyExclude(relPath, excludes))) {
    ...
    async generateLibRegistry() {
        ...
        for (const library of this.config.libraries) {
            const entry = {
                ...
                ...(library.symlinks && Object.keys(library.symlinks).length > 0 ? { symlinks: library.symlinks } : {}),
