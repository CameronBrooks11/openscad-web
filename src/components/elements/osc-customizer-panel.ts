...
export class OscCustomizerPanel extends LitElement {
  ...
  override render() {
    ...
    const groups = (st.parameterSet?.parameters ?? []).reduce(
      (acc, p) => {
    ...
    const setOpen = (group: string, open: boolean) => {
      ...
      this._model.mutate((s2) => {
        s2.view.collapsedCustomizerTabs = Array.from(s);
      });
    };
  ...
  private _renderControl(param: Parameter, value: any) {
    ...
    if (param.type === 'number' && 'options' in param && param.options) {
      return html`
        ...
        >
          ${(param.options as ParameterOption[]).map(
            (o) => html`
    ...
    if (param.type === 'number') {
      return html`
        <input
          ...
          .value=${String(v)}
          @change=${(e: Event) =>
            this._handleChange(param.name, Number((e.target as HTMLInputElement).value))}
        />
      `;
    }
    ...
    if (param.type === 'string') {
      return html`
        <input
          ...
          .value=${String(v)}
          @input=${(e: Event) =>
            this._handleChange(param.name, (e.target as HTMLInputElement).value)}
        />
      `;
    }
        ...
        <div class="slider-row">
          <input
            ...
            @input=${(e: Event) =>
              this._handleChange(param.name, Number((e.target as HTMLInputElement).value))}
          />
          <span style="font-size:0.8rem;width:40px;text-align:right;"
            >${Number(v).toFixed(Number((param as { step?: number }).step ?? 1) < 1 ? 2 : 0)}</span
          >
        </div>
      `;
    }
