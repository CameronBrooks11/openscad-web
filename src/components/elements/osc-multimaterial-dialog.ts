...
export class OscMultimaterialDialog extends LitElement {
  static override styles = css`
    ...
    .header {
      padding: 12px 16px; font-weight: bold; font-size: 1rem;
      border-bottom: 1px solid #eee;
    }
    .body { padding: 16px; }
    p { margin: 0 0 8px; font-size: 0.875rem; color: #555; }
    .color-row { display: flex; align-items: center; gap: 8px; margin-bottom: 8px; }
    input[type="color"] { width: 36px; height: 28px; padding: 0; border: 1px solid #ccc; cursor: pointer; border-radius: 2px; }
    input[type="text"] { flex: 1; padding: 4px 8px; border: 1px solid #ccc; border-radius: 4px; font-size: 0.875rem; }
    input[type="text"].invalid { border-color: #e00; background: #fff5f5; }
    button.remove { background: none; border: none; cursor: pointer; color: #e00; font-size: 1rem; padding: 0 4px; }
    button.remove:hover { color: #900; }
    button.add { margin-top: 4px; padding: 4px 12px; border: 1px solid #aaa; background: #f5f5f5; border-radius: 4px; cursor: pointer; font-size: 0.85rem; }
    button.add:disabled { opacity: 0.5; cursor: not-allowed; }
    .footer {
      padding: 10px 16px; border-top: 1px solid #eee;
      display: flex; align-items: center; justify-content: space-between;
    }
    .footer-left { display: flex; align-items: center; gap: 8px; font-size: 0.85rem; }
    .footer-right { display: flex; gap: 8px; }
    button.btn { padding: 5px 14px; border: 1px solid #bbb; border-radius: 4px; cursor: pointer; font-size: 0.85rem; background: #f5f5f5; }
    button.btn-primary { background: #4f87c5; color: #fff; border-color: #4f87c5; }
    button.btn-primary:disabled { opacity: 0.5; cursor: not-allowed; }
    button.btn:hover:not(:disabled) { background: #e8e8e8; }
    button.btn-primary:hover:not(:disabled) { background: #3a72b0; }
  `;
  ...
  private _cancel() {
    this._model.mutate(s => { s.view.extruderPickerVisibility = undefined; });
  }
  ...
  override render() {
    ...
    return html`
      <dialog @cancel=${(e: Event) => { e.preventDefault(); this._cancel(); }}>
        <div class="header">Multimaterial Color Picker</div>
        <div class="body">
          <p>To print on a multimaterial printer, we map the model's colors to the nearest extruder color.</p>
          <p>Define the colors of your extruders below.</p>
          ${this._tempColors.map((color, i) => html`
            <div class="color-row">
              <input type="color"
                .value=${chroma.valid(color) ? chroma(color).hex() : '#000000'}
                @change=${(e: Event) => this._setColor(i, chroma((e.target as HTMLInputElement).value).name())} />
              <input type="text"
                class=${color.trim() !== '' && !chroma.valid(color) ? 'invalid' : ''}
                .value=${color}
                ?autofocus=${color === ''}
                @keydown=${(e: KeyboardEvent) => { if (e.key === 'Enter' && canAdd) { e.preventDefault(); this._addColor(); } }}
                @input=${(e: Event) => {
                  let v = (e.target as HTMLInputElement).value.trim();
                  try { v = chroma(v).name(); } catch { /* keep raw */ }
                  this._setColor(i, v);
                }} />
        ...
        <div class="footer">
          <div class="footer-left">
            <input type="checkbox"
              ...
              @change=${(e: Event) => this._model.mutate(s => { s.params.skipMultimaterialPrompt = (e.target as HTMLInputElement).checked; })} />
