...
export class OscPanelSwitcher extends LitElement {
  static override styles = css`
    :host { display: block; }
    .bar {
      display: flex; flex-direction: row; margin: 5px;
      position: relative; align-items: center;
    }
    .tabs { display: flex; flex: 1; gap: 4px; justify-content: center; }
    button.tab {
      padding: 5px 14px; border: 1px solid #ccc; border-radius: 4px;
      background: #f5f5f5; cursor: pointer; font-size: 0.875rem; color: #333;
    }
    button.tab:hover { background: #e8e8e8; }
    button.tab.active { background: #4f87c5; color: #fff; border-color: #4f87c5; }
    button.tab.toggled { background: #4f87c5; color: #fff; border-color: #4f87c5; }
    button.tab.untoggled { background: #f5f5f5; color: #555; }
  `;
        ...
        <div class="tabs">
          ${layout.mode === 'multi'
            ? targets.map(({ id, label, title }) => {
                ...
                return html`
                  <button class="tab ${on ? 'toggled' : 'untoggled'}"
                    title="${title}"
            ...
            : targets.map(({ id, label, title }) => html`
                <button class="tab ${layout.focus === id ? 'active' : ''}"
                  title="${title}"
