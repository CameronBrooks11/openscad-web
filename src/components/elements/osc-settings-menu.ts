...
export class OscSettingsMenu extends LitElement {
  static override styles = css`
    ...
    button.item {
      ...
      text-align: left; white-space: nowrap; font-size: 0.9rem;
      color: #333;
    }
    button.item:hover { background: #f0f0f0; }
    hr { margin: 4px 0; border: none; border-top: 1px solid #eee; }
    button.item.danger { color: #c00; }
  `;
  ...
  override render() {
    ...
    return html`
      <details>
        ...
        <div class="menu">
          <button class="item" @click=${() => this._model.changeLayout(st.view.layout.mode === 'multi' ? 'single' : 'multi')}>
            ${st.view.layout.mode === 'multi' ? 'Switch to single panel mode' : 'Switch to side-by-side mode'}
          </button>
          <hr />
          <button class="item" @click=${() => this._model.mutate(s => { s.view.showAxes = !s.view.showAxes; })}>
            ${st.view.showAxes ? 'Hide axes' : 'Show axes'}
          </button>
          <button class="item" @click=${() => this._model.mutate(s => { s.view.lineNumbers = !s.view.lineNumbers; })}>
            ${st.view.lineNumbers ? 'Hide line numbers' : 'Show line numbers'}
          </button>
          <button class="item" @click=${() => this._model.mutate(s => { s.params.autoCompile = !(s.params.autoCompile ?? true); })}>
            ${(st.params.autoCompile ?? true) ? 'Disable auto-compile on edit' : 'Enable auto-compile on edit'}
          </button>
          <button class="item" @click=${() => this._model.mutate(s => { s.view.customizerGroupsCollapsed = !(s.view.customizerGroupsCollapsed ?? false); })}>
            ${(st.view.customizerGroupsCollapsed ?? false) ? 'Expand customizer groups by default' : 'Collapse customizer groups by default'}
          </button>
          <hr />
          <button class="item" @click=${() => this._model.mutate(s => { s.params.backend = s.params.backend === 'cgal' ? 'manifold' : 'cgal'; })}>
