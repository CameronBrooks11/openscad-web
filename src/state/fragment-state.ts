...
function validateVars(v: unknown): State['params']['vars'] {
  ...
  return Object.fromEntries(
    Object.entries(v as Record<string, unknown>).filter(([k]) => typeof k === 'string' && k.length > 0),
  );
}
    ...
    try {
      ...
      return {
        params: {
          ...
          sources: validateSources(params?.sources ?? (params?.source ? [{path: params?.sourcePath, content: params?.source}] : undefined)),
